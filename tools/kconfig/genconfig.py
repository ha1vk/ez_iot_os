...
def _cmake_contents(kconfig, header, kvlist):
    chunks = [header]
    add = chunks.append
    config_vars = []
    ...
    for kv in kvlist:
        add("set({}{} \"{}\")\n".format(
            kconfig.config_prefix, kv[0], kv[1]))

    return "".join(chunks)

def _make_contents(kconfig, header, kvlist):
    chunks = [header]
    add = chunks.append
    config_vars = []
    for sym in kconfig.unique_defined_syms:
        ...
        elif sym.orig_type in (kconfiglib.STRING, kconfiglib.TRISTATE):
            add("{}{}=\"{}\"\n".format(kconfig.config_prefix, sym.name, val))
        else:
            add("{}{}={}\n".format(kconfig.config_prefix, sym.name, val))
        config_vars.append(str(kconfig.config_prefix+sym.name))
    ...
    for kv in kvlist:
        add("{}{}={}\n".format(kconfig.config_prefix, kv[0], kv[1]))

    return "".join(chunks)

def write_config(kconfig, filename, gui, kvlist):
    print("-- Write config at: " + filename)
    if not gui:
        kconfig.write_config(filename)

def write_make(kconfig, filename, gui, kvlist):
    print("-- Write makefile mk at: " + filename)
    make_conf_header = "# Generated by Kconfiglib (https://github.com/ulfalizer/Kconfiglib)\n"
    make_conf_header += "### DO NOT edit this file!! ###\n\n"
    make_conf_content = _make_contents(kconfig, make_conf_header, kvlist)
    # don't change file info if config no change#
    if os.path.exists(filename):
        with open(filename) as f:
            if f.read() == make_conf_content:
                return
    f = open(filename, "w")
    f.write(make_conf_content)
    f.close()

def write_cmake(kconfig, filename, gui, kvlist):
    print("-- Write  cmake  config  at: " + filename)
    cmake_conf_header = "# Generated by Kconfiglib (https://github.com/ulfalizer/Kconfiglib)\n"
    cmake_conf_header += "### DO NOT edit this file!! ###\n\n"
    cmake_conf_content = _cmake_contents(kconfig, cmake_conf_header, kvlist)
    # don't change file info if config no change
    if os.path.exists(filename):
        with open(filename) as f:
...
def write_header(kconfig, filename, gui, kvlist):
...
parser.add_argument('--addkv', nargs=2, action='append',
                    help='Write output file (format and output filename)',
                    metavar=('key', 'value'),
                    default=[])
...
for fmt, filename in out_format.items():
    ...
    func(kconfig, filename, args.menuconfig == "True", args.addkv)
