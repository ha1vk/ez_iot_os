...
{
    memset( ctx, 0, sizeof( bscomptls_md4_context ) );
}
...
void bscomptls_md4_free( bscomptls_md4_context *ctx )
{
    ...
    bscomptls_zeroize( ctx, sizeof( bscomptls_md4_context ) );
}
...
#if !defined(BSCOMPTLS_MD4_PROCESS_ALT)
...
#endif /* !BSCOMPTLS_MD4_PROCESS_ALT */
...
void bscomptls_md4_update( bscomptls_md4_context *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        memcpy( (void *) (ctx->buffer + left),
                (void *) input, fill );
        bscomptls_md4_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 64 )
    {
        bscomptls_md4_process( ctx, input );
        input += 64;
        ilen  -= 64;
    }
...
void bscomptls_md4_finish( bscomptls_md4_context *ctx, unsigned char output[16] )
{
    ...
    bscomptls_md4_update( ctx, (unsigned char *) md4_padding, padn );
    bscomptls_md4_update( ctx, msglen, 8 );
...
{
    bscomptls_md4_context ctx;

    bscomptls_md4_init( &ctx );
    bscomptls_md4_starts( &ctx );
    bscomptls_md4_update( &ctx, input, ilen );
    bscomptls_md4_finish( &ctx, output );
    bscomptls_md4_free( &ctx );
}
...
int bscomptls_md4_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            bscomptls_printf( "  MD4 test #%d: ", i + 1 );

        bscomptls_md4( (unsigned char *) md4_test_str[i],
