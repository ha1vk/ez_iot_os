...
{
    memset( ctx, 0, sizeof( bscomptls_md2_context ) );
}
...
void bscomptls_md2_free( bscomptls_md2_context *ctx )
{
    ...
    bscomptls_zeroize( ctx, sizeof( bscomptls_md2_context ) );
}
...
#if !defined(BSCOMPTLS_MD2_PROCESS_ALT)
...
#endif /* !BSCOMPTLS_MD2_PROCESS_ALT */
...
{
    bscomptls_md2_context ctx;

    bscomptls_md2_init( &ctx );
    bscomptls_md2_starts( &ctx );
    bscomptls_md2_update( &ctx, input, ilen );
    bscomptls_md2_finish( &ctx, output );
    bscomptls_md2_free( &ctx );
}
...
int bscomptls_md2_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            bscomptls_printf( "  MD2 test #%d: ", i + 1 );

        bscomptls_md2( (unsigned char *) md2_test_str[i],
