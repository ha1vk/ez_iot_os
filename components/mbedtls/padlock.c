                       ...
                       unsigned char output[16] )
{
    ...
    rk  = ctx->rk;
    blk = BSCOMPTLS_PADLOCK_ALIGN16( buf );
    memcpy( blk, input, 16 );
                       ...
                       unsigned char *output )
{
    ...
    if( ( (long) input  & 15 ) != 0 ||
        ( (long) output & 15 ) != 0 )
        return( BSCOMPTLS_ERR_PADLOCK_DATA_MISALIGNED );

    rk = ctx->rk;
    iw = BSCOMPTLS_PADLOCK_ALIGN16( buf );
    memcpy( iw, iv, 16 );
...
#endif /* BSCOMPTLS_HAVE_X86 */

#endif /* BSCOMPTLS_PADLOCK_C */
