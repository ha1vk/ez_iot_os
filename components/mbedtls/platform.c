...
int bscomptls_platform_std_nv_seed_read( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( fd = ez_open( BSCOMPTLS_PLATFORM_STD_NV_SEED_FILE, O_RDONLY|O_CREAT, 0x777) ) == -1 )
        return -1;

    if( ( n = ez_read(fd, buf,  buf_len ) ) != buf_len )
    {
        ez_close( fd );
        return -1;
    }
...
int bscomptls_platform_std_nv_seed_write( unsigned char *buf, size_t buf_len )
{
    ...
    if( ( fd = ez_open( BSCOMPTLS_PLATFORM_STD_NV_SEED_FILE, O_WRONLY|O_CREAT|O_TRUNC, 0x777) ) == -1 )
        return -1;

    if( ( n = ez_write(fd, buf, buf_len ) ) != buf_len )
    {
        ez_close( fd );
        return -1;
    }
