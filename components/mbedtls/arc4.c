...
{
    memset( ctx, 0, sizeof( bscomptls_arc4_context ) );
}
...
void bscomptls_arc4_free( bscomptls_arc4_context *ctx )
{
    ...
    bscomptls_zeroize( ctx, sizeof( bscomptls_arc4_context ) );
}
...
int bscomptls_arc4_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            bscomptls_printf( "  ARC4 test #%d: ", i + 1 );

        memcpy( ibuf, arc4_test_pt[i], 8 );

        bscomptls_arc4_setup( &ctx, arc4_test_key[i], 8 );
        bscomptls_arc4_crypt( &ctx, 8, ibuf, obuf );
