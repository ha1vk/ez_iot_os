...
{
    memset( ctx, 0, sizeof( bscomptls_md5_context ) );
}
...
void bscomptls_md5_free( bscomptls_md5_context *ctx )
{
    ...
    bscomptls_zeroize( ctx, sizeof( bscomptls_md5_context ) );
}
...
#if !defined(BSCOMPTLS_MD5_PROCESS_ALT)
...
#endif /* !BSCOMPTLS_MD5_PROCESS_ALT */
...
void bscomptls_md5_update( bscomptls_md5_context *ctx, const unsigned char *input, size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        bscomptls_md5_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 64 )
    {
        bscomptls_md5_process( ctx, input );
        input += 64;
        ilen  -= 64;
    }
...
void bscomptls_md5_finish( bscomptls_md5_context *ctx, unsigned char output[16] )
{
    ...
    bscomptls_md5_update( ctx, md5_padding, padn );
    bscomptls_md5_update( ctx, msglen, 8 );
...
{
    bscomptls_md5_context ctx;

    bscomptls_md5_init( &ctx );
    bscomptls_md5_starts( &ctx );
    bscomptls_md5_update( &ctx, input, ilen );
    bscomptls_md5_finish( &ctx, output );
    bscomptls_md5_free( &ctx );
}
...
int bscomptls_md5_self_test( int verbose )
{
    ...
    {
        if( verbose != 0 )
            bscomptls_printf( "  MD5 test #%d: ", i + 1 );

        bscomptls_md5( md5_test_buf[i], md5_test_buflen[i], md5sum );
