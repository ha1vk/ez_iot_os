...
#ifndef UTEST_CASE_MAX_NUM
    #define UTEST_CASE_MAX_NUM (128u)
#endif

#ifndef UTEST_CASE_CONSTRUCTOR_INIT
    #define UTEST_CASE_CONSTRUCTOR_INIT (102)
#endif

int utest_init(void);
static struct utest_tc_export _utest_testcase[UTEST_CASE_MAX_NUM];
__attribute__((constructor(UTEST_CASE_CONSTRUCTOR_INIT))) void uctest_init(void)
{
    utest_init();
}

void uctest_insert_table(tc_fun_t testcase, const char *name, tc_con_t init, tc_con_t cleanup, uint32_t timeout)
{
    tc_table[tc_num].name = name;
    tc_table[tc_num].tc = testcase;
    tc_table[tc_num].init = init;
    tc_table[tc_num].cleanup = cleanup;
    tc_table[tc_num].run_timeout = timeout;
    tc_num++;
    if (tc_num >= UTEST_CASE_MAX_NUM)
    {
        printf("tc_num init faile(%ld)\n", tc_num);
    }
}
#endif
...
int utest_init(void)
{
    ...
    LOG_I("total utest testcase num: (%ld)", tc_num);
    return tc_num;
}
