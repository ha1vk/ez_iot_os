...
{
    memset( ctx, 0, sizeof( bscomptls_sha512_context ) );
}
...
void bscomptls_sha512_free( bscomptls_sha512_context *ctx )
{
    ...
    bscomptls_zeroize( ctx, sizeof( bscomptls_sha512_context ) );
}
...
#if !defined(BSCOMPTLS_SHA512_PROCESS_ALT)
...
#endif /* !BSCOMPTLS_SHA512_PROCESS_ALT */
                    ...
                    size_t ilen )
{
    ...
    if( left && ilen >= fill )
    {
        ...
        bscomptls_sha512_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
    ...
    while( ilen >= 128 )
    {
        bscomptls_sha512_process( ctx, input );
        input += 128;
        ilen  -= 128;
    }
...
void bscomptls_sha512_finish( bscomptls_sha512_context *ctx, unsigned char output[64] )
{
    ...
    bscomptls_sha512_update( ctx, sha512_padding, padn );
    bscomptls_sha512_update( ctx, msglen, 16 );
...
{
    bscomptls_sha512_context ctx;

    bscomptls_sha512_init( &ctx );
    bscomptls_sha512_starts( &ctx, is384 );
    bscomptls_sha512_update( &ctx, input, ilen );
    bscomptls_sha512_finish( &ctx, output );
    bscomptls_sha512_free( &ctx );
}
...
int bscomptls_sha512_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( verbose != 0 )
            bscomptls_printf( "  SHA-%d test #%d: ", 512 - k * 128, j + 1 );

        bscomptls_sha512_starts( &ctx, k );
        ...
        if( j == 2 )
        {
            ...
            for( j = 0; j < 1000; j++ )
                bscomptls_sha512_update( &ctx, buf, buflen );
        }
        else
            bscomptls_sha512_update( &ctx, sha512_test_buf[j],
                                 sha512_test_buflen[j] );

        bscomptls_sha512_finish( &ctx, sha512sum );
