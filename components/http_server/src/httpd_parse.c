...
static ez_err_t verify_url(http_parser *parser)
{
...
//    }
    ...
    {
        ezlog_w(TAG, LOG_FMT("unsupported HTTP version = %d.%d"),
 ...
 */
static ez_err_t cb_url(http_parser *parser,
                       const char *at, size_t length)
{
    ...
    {
        ezlog_v(TAG, LOG_FMT("message begin"));
    ...
    {
        ezlog_e(TAG, LOG_FMT("unexpected state transition"));
        parser_data->status = PARSING_FAILED;
        return EZHTTPD_ERRNO_FAIL;
    }
...
static ez_err_t pause_parsing(http_parser *parser, const char *at)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
 ...
 */
static ez_err_t cb_header_field(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    {
        if (verify_url(parser) != EZHTTPD_ERRNO_SUCC)
        {
            parser_data->status = PARSING_FAILED;
            return EZHTTPD_ERRNO_FAIL;
        }

        ezlog_v(TAG, LOG_FMT("headers begin"));
        ...
        if (pause_parsing(parser, at) != EZHTTPD_ERRNO_SUCC)
        {
            parser_data->status = PARSING_FAILED;
            return EZHTTPD_ERRNO_FAIL;
        }
 ...
 */
static ez_err_t cb_header_value(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
 ...
 */
static ez_err_t cb_headers_complete(http_parser *parser)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    struct httpd_req *r = parser_data->req;
    struct httpd_req_aux *ra = r->aux;
    ...
    if (parser_data->status == PARSING_URL)
    {
        ezlog_v(TAG, LOG_FMT("no headers"));
        if (verify_url(parser) != EZHTTPD_ERRNO_SUCC)
        {
            parser_data->status = PARSING_FAILED;
            return EZHTTPD_ERRNO_FAIL;
    ...
    if (parser->upgrade)
    {
        ezlog_w(TAG, LOG_FMT("upgrade from HTTP not supported"));
 ...
 * Will be invoked ONLY once every packet
 */
static ez_err_t cb_on_body(http_parser *parser, const char *at, size_t length)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    ...
    ezlog_v(TAG, LOG_FMT("body begins"));
    return EZHTTPD_ERRNO_SUCC;
}
 ...
 * Will be invoked ONLY once every packet
 */
static ez_err_t cb_no_body(http_parser *parser)
{
    parser_data_t *parser_data = (parser_data_t *)parser->data;
    const char *at = parser_data->last.at;
    ...
    parser_data->last.length = 0;
    parser_data->status = PARSING_COMPLETE;
    ezlog_v(TAG, LOG_FMT("message complete"));
    return EZHTTPD_ERRNO_SUCC;
}
...
static int read_block(httpd_req_t *req, size_t offset, size_t length)
{
    ...
    {
        ezlog_v(TAG, LOG_FMT("connection closed"));
        return -1;
    }

    ezlog_v(TAG, LOG_FMT("received HTTP request block size = %d"), nbytes);
    return nbytes;
}
...
static int parse_block(http_parser *parser, size_t offset, size_t length)
{
    ...
    if (!length)
    {
        ezlog_w(TAG, LOG_FMT("response uri/header too big"));
        switch (data->status)
        {
        case PARSING_URL:
    ...
    {
        ezlog_w(TAG, LOG_FMT("parsing failed"));
        return -1;
    }
    else if (data->paused)
    {
 ...
 */
static ez_err_t httpd_parse_req(struct httpd_data *hd)
{
 ...
 */
ez_err_t httpd_req_new(struct httpd_data *hd, struct sock_db *sd)
{
    httpd_req_t *r = &hd->hd_req;
    ...
    /* Parse request */
    ez_err_t err = httpd_parse_req(hd);
    if (err != EZHTTPD_ERRNO_SUCC)
    {
        httpd_req_cleanup(r);
    }
    return err;
 ...
 */
ez_err_t httpd_req_delete(struct httpd_data *hd)
{
    httpd_req_t *r = &hd->hd_req;
    struct httpd_req_aux *ra = r->aux;
    ...
    while (ra->remaining_len)
    {
        ...
        dummy[ret] = '\0';
        ezlog_v(TAG, LOG_FMT("purging data : %s"), dummy);
    }
...
ez_err_t httpd_query_key_value(const ez_int8_t *qry_str, const ez_int8_t *key, ez_int8_t *val, ez_size_t val_size)
{
    ...
    ezlog_v(TAG, LOG_FMT("key %s not found"), key);
