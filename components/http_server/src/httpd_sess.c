...
ez_err_t httpd_sess_new(struct httpd_data *hd, int newfd)
{
    ezlog_v(TAG, LOG_FMT("fd = %d"), newfd);

    if (httpd_sess_get(hd, newfd))
    {
        ezlog_e(TAG, LOG_FMT("session already exists with fd = %d"), newfd);
    ...
    for (i = 0; i < hd->config.max_open_sockets; i++)
    {
        if (hd->hd_sd[i].fd == -1)
        {
            ...
            {
                ez_err_t ret = hd->config.open_fn(hd, hd->hd_sd[i].fd);
    ...
    ezlog_v(TAG, LOG_FMT("unable to launch session for fd = %d"), newfd);
    return EZHTTPD_ERRNO_FAIL;
}
...
void httpd_sess_delete_invalid(struct httpd_data *hd)
{
    for (int i = 0; i < hd->config.max_open_sockets; i++)
    {
        ...
        {
            ezlog_w(TAG, LOG_FMT("Closing invalid socket %d"), hd->hd_sd[i].fd);
...
/* This MUST return EZHTTPD_ERRNO_SUCC on successful execution. If any other
 * value is returned, everything related to this socket will be
 * cleaned up and the socket will be closed.
 */
ez_err_t httpd_sess_process(struct httpd_data *hd, int newfd)
{
    ...
    ezlog_v(TAG, LOG_FMT("httpd_req_new"));
    if (httpd_req_new(hd, sd) != EZHTTPD_ERRNO_SUCC)
    {
        return EZHTTPD_ERRNO_FAIL;
    }
    ezlog_v(TAG, LOG_FMT("httpd_req_delete"));
    if (httpd_req_delete(hd) != EZHTTPD_ERRNO_SUCC)
    {
        return EZHTTPD_ERRNO_FAIL;
    }
    ezlog_v(TAG, LOG_FMT("success"));
    sd->lru_counter = httpd_sess_get_lru_counter();
    return EZHTTPD_ERRNO_SUCC;
}

ez_err_t httpd_sess_update_lru_counter(httpd_handle_t handle, int sockfd)
{
    if (handle == NULL)
    {
        return EZHTTPD_ERRNO_FAIL;
...
ez_err_t httpd_sess_close_lru(struct httpd_data *hd)
{
    ...
    ezlog_v(TAG, LOG_FMT("fd = %d"), lru_fd);
...
ez_err_t httpd_sess_trigger_close(httpd_handle_t handle, int sockfd)
{
