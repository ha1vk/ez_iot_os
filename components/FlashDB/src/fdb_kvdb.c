...
static size_t get_kv(fdb_kvdb_t db, const char *key, void *value_buf, size_t buf_len, size_t *value_len)
{
    ...
    if (find_kv(db, key, &kv))
    {
        ...
        if (value_buf)
        {
            if (FDB_NO_ERR != _fdb_flash_read((fdb_db_t)db, kv.addr.value, (uint32_t *)value_buf, read_len))
            {
                read_len = -1;
            }
        }
...
static bool alloc_kv_cb(kv_sec_info_t sector, void *arg1, void *arg2)
{
    ...
    if (sector->check_ok && sector->remain > arg->kv_size && ((sector->status.dirty == FDB_SECTOR_DIRTY_FALSE) || (sector->status.dirty == FDB_SECTOR_DIRTY_TRUE && !arg->db->gc_request)))
    {
