...
static mkernel_internal_error wait_assign_response(ezdev_sdk_kernel *sdk_kernel, lbs_affair *authi_affair, EZDEV_SDK_UINT32 *rev_cmd, EZDEV_SDK_UINT32 *remain_len)
{
	...
	mkernel_internal_error sdk_error = sdk_kernel->platform_handle.net_work_read(authi_affair->socket_fd, &byte_1, 1, 5 * 1000);
	...
	*remain_len = 0;
	...
	{
		byte_2 = 0;
		sdk_error = sdk_kernel->platform_handle.net_work_read(authi_affair->socket_fd, &byte_2, 1, 5 * 1000);
		if (sdk_error != mkernel_internal_succ)
		{
    ...
    if (0x01 == flag && DEV_PROTOCOL_AUTHENTICATION_II == *rev_cmd)
    {
        ...
        sdk_error = sdk_kernel->platform_handle.net_work_read(authi_affair->socket_fd, authi_affair->global_in_packet.var_head_buf, 1, 5 * 1000);
        if (sdk_error != 0)
        {
	...
	sdk_error = sdk_kernel->platform_handle.net_work_read(authi_affair->socket_fd, authi_affair->global_in_packet.payload_buf, *remain_len, 5 * 1000);
...
static mkernel_internal_error send_lbs_msg(ezdev_sdk_kernel *sdk_kernel, lbs_affair *authi_affair)
{
    ...
    unsigned char cmd = 0;
    unsigned char flag = 0;
	result_ = sdk_kernel->platform_handle.net_work_write(authi_affair->socket_fd, authi_affair->global_out_packet.head_buf, authi_affair->global_out_packet.head_buf_off, 5 * 1000, &real_sendlen);
    ...
    {
        result_ = sdk_kernel->platform_handle.net_work_write(authi_affair->socket_fd, authi_affair->global_out_packet.var_head_buf, authi_affair->global_out_packet.var_head_buf_off, 5 * 1000, &real_sendlen);
        if (result_ != mkernel_internal_succ)
        {
            return result_;
        }
	...
	result_ = sdk_kernel->platform_handle.net_work_write(authi_affair->socket_fd, authi_affair->global_out_packet.payload_buf, authi_affair->global_out_packet.payload_buf_off, 5 * 1000, &real_sendlen);
	if (result_ != mkernel_internal_succ)
	{
		return result_;
	}
...
static mkernel_internal_error lbs_connect(ezdev_sdk_kernel *sdk_kernel, lbs_affair *authi_affair)
{
	...
	do
	{
		authi_affair->socket_fd = sdk_kernel->platform_handle.net_work_create(g_binding_nic);
		...
		result_ = sdk_kernel->platform_handle.net_work_connect(authi_affair->socket_fd, sdk_kernel->server_info.server_name, sdk_kernel->server_info.server_port, 5 * 1000, szRealIp);
		if (result_ != mkernel_internal_succ)
		{
			...
			sdk_kernel->platform_handle.net_work_disconnect(authi_affair->socket_fd);
			...
			result_ = sdk_kernel->platform_handle.net_work_connect(authi_affair->socket_fd, sdk_kernel->server_info.server_ip, sdk_kernel->server_info.server_port, 5 * 1000, szRealIp);
			if (result_ != mkernel_internal_succ)
			{
