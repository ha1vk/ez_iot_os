...
extern "C"
{
    ...
    typedef struct
    {
        ez_int_t retry_max;          ///< 下载最大重试次数
        ez_int_t file_num;           ///< 升级信息数量
        ez_int_t interval;           ///< 上报升级进度间隔
    ...
    {
        ez_int_t interval;       ///< 上报升级进度间隔
        ez_int_t file_num;       //V3升级协议中升级文件个数，
        ez_char_t mod_name[32];  //ptid
        ez_int_t bSubdevUpgrade; //至少包含有一个升级包程序
    } ez_ota_data_t;

    typedef struct
    {
        ez_int8_t ota_label[8];
        ez_int8_t ota_code;
    } ez_ota_t; 
    ...
    ez_int_t ez_ota_init();

    ez_void_t ez_ota_start();
