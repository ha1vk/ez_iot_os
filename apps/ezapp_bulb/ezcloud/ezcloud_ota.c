...
static ez_int32_t download_data_cb(ez_int32_t total_len, ez_int32_t offset, ez_void_t *data, ez_int32_t len, ez_void_t *user_data)
{
    ...
    ez_ota_upgrade_info_t *pStruUserOtaData = (ez_ota_upgrade_info_t *)user_data;
    /*第一次往flash 写数据，先清空数据，这里需要消耗一定时间*/
    if (0 == offset) //
    {
        ezlog_i(TAG_OTA, "starting ota to partition ,first erase partition");
        ez_iot_ota_progress_report(NULL, pStruUserOtaData->pota_files->mod_name, OTA_STATE_DOWNLOADING, 0);
    ...
    */
    ...
    {
        ez_iot_ota_progress_report(NULL, pStruUserOtaData->pota_files->mod_name, OTA_STATE_DOWNLOADING, progress);
    }
    if (progress == 100)
    {
        rv = 0;
...
static ez_void_t download_result_cb(ez_ota_cb_result_e result, ez_void_t *user_data)
{
    ...
    {
        ez_iot_ota_status_fail(&pres, pStruUserOtaData->pota_files->mod_name, "", OTA_STATE_DOWNLOADING); //上报下载过程中升级错误的
    }
    ...
    if (OK != rv)
    {
        ota_code = REBOOT_OTA_FAILED;
        ezlog_e(TAG_OTA, "one module update,ota_set_boot_partition failed! err=0x%x", rv);
    }
    else
    {
        ota_code = REBOOT_OTA_SUCCEED;
        ezlog_i(TAG_OTA, "one module update,wifidev change partition scucess..");
    }
    hal_config_set_int("ota_code", ota_code);
    ez_iot_ota_progress_report(&pres, pStruUserOtaData->pota_files->mod_name, OTA_STATE_REBOOTING, 100); //第二次进来，wifi 下载完毕，设备需要重启
    ez_iot_ota_progress_report(&pres, pStruUserOtaData->pota_files->mod_name, OTA_STATE_REBOOTING, 100); //第二次进来，wifi 下载完毕，设备需要重启
    ez_iot_ota_deinit();
...
static ez_int32_t ota_download_fun(ez_ota_upgrade_info_t *upgrade_infos, ez_int32_t file_index)
{
    ...
    ez_ota_upgrade_info_t *pstruUserOtaData = (ez_ota_upgrade_info_t *)ezos_malloc(sizeof(ez_ota_upgrade_info_t));
    if (NULL == pstruUserOtaData)
    {
        ezlog_e(TAG_OTA, "malloc download userdate failed!");
        return OTA_CODE_MEM;
    }
    memset(pstruUserOtaData, 0, sizeof(ez_ota_file_info_t));
    ...
    strncpy(pstruUserOtaData->pota_files->mod_name, upgrade_infos->pota_files[file_index].mod_name, sizeof(pstruUserOtaData->pota_files->mod_name));
...
static ez_int32_t ota_event_notify(ez_ota_res_t *pres, ez_ota_event_e event, ez_void_t *data, ez_int32_t len)
{
    ...
    case START_UPGRADE:
    {
        ...
        rv = ota_download_fun((ez_ota_upgrade_info_t *)upgrade_infos, 0);
        if (0 != rv)
        {
    ...
    ota_code = REBOOT_OTA_FAILED; //初始状态先写成失败
    hal_config_set_int("ota_code", ota_code);
    return rv;
}
...
ez_int32_t ez_ota_reboot_report_result()
{
    ez_ota_res_t pres = {0};
    ez_int8_t ota_code;
    if (0 != hal_config_get_int("ota_code", &ota_code, 2))
    {
        ezlog_e(TAG_APP, "config_read ota_code error!");
        return -1;
    }
    if (REBOOT_NORMAL == ota_code)
    {
    ...
    {
        /* 报升级成功与否*/
        if (REBOOT_OTA_SUCCEED == ota_code)
        {
